...
| 14 | **Deploy: Sovereign Pilot (self-host)** | Planned (brainstormed 2026-08-10) | Single-node egress-locked **docker-compose** bundle; **in-boundary helper models** (bundled Ollama serves router + embeddings — closes TD-009); egress-lock by construction (internal network) + **egress probe in the attestation** (provable zero-egress); one-command up + `doctor` + runbook. Pilot slice — defers Helm/Postgres/Vault/air-gap/HA/SCIM. Spec: `specs/phases/phase-14-deploy-pilot/`. |
