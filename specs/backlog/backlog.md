...
| FEAT-011 | **Cache & compression v2** — two tabs on one page; **per-endpoint** (not global) config; pluggable strategy registry (multiple cache algos / compression methods) + **bring-your-own** | P2 | open | (demo feedback) | Demo feedback #2/#3/#4. Fits ports & adapters (CachePort/CompressionPort). Per-endpoint changes the metering/savings model. BYO = design-for-now, build-later. Do AFTER the naming settles (renames backend→endpoint). |
