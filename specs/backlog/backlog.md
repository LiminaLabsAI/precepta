...
| TD-009 | Router model must run **in-boundary** in production (sovereignty) | P1 | open | **8 (deploy)** | Precepta's router/intent model (`.env` HF, `app/router/intent.py`) reads the customer's prompt to classify it. In dev it points at `router.huggingface.co` (public) — prompts leave the boundary, breaking zero-egress. **RESOLVE AT DEPLOY (per user 2026-08-01):** the deployment strategy must ship/deploy Precepta's router model as a dedicated **in-boundary** endpoint (customer's network), and `.env` HF points there. Same applies to any other helper model (embeddings for semantic cache, eval judge). Bring this concern back when scoping Phase 8. |
